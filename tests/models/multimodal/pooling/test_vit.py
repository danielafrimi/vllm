...
from transformers import AutoConfig, AutoModel, CLIPImageProcessor, PretrainedConfig
from vllm.model_executor.models.intern_vit import InternVisionModel, RadioModel
...
def build_intern_config_from_radio(radio_cfg):
    ...
    hidden_size, num_layers, num_heads, intermediate_size = vit_dims.get(
        model_name, (768, 12, 12, 3072)  # safe default (ViT-B/16)
    )
...
def map_hf_radio_to_vllm_intern(hf_sd: dict, radio_vllm) -> dict:
    ...
    for k, v in hf_sd.items():
        ...
        if k2.startswith("model.blocks."):
            parts = k2.split(".")
            layer_idx = parts[2]
            suffix = ".".join(parts[3:])  # e.g. norm1.weight, attn.qkv.weight, mlp.fc1.weight, etc.
...
def _test_radio_vllm_vs_hf():
    ...
    images = [Image.open('/home/dafrimi/projects/vllm/images/horse.jpg').convert('RGB')]

    pixel_values = [
        image_processor(images, return_tensors='pt').pixel_values.to(hf_model.dtype)[: , : , :432, :640]
    ...
    try:
        ...
        hidden_size, num_layers, num_heads, intermediate_size = get_args_from_model_type(config.args["model"])
        ...
        vllm_state_dict = map_hf_radio_to_vllm_intern(hf_state_dict, radio_vllm)
        missing, unexpected = radio_vllm.load_state_dict(vllm_state_dict, strict=False)
        print(f"missing: {missing}")
        print(f"unexpected: {unexpected}")

        vllm_outputs_per_image = [
        radio_vllm(pixel_values=pixel_value.to("cuda")) for pixel_value in pixel_values]
