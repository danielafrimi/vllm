...
BENCH_ARGS="${BENCH_ARGS:---correctness --bench --key-len 131072 --q-len 8192 --query-chunk-size 512 --top-chunks 1024 --warmup 2 --iters 5}"
...
PY
    ...
    python3 /workspace/vllm-src/tools/bench_dsa_moonshot_prefill.py \
      '"${BENCH_ARGS}"' \
      --json-out /logs/moonshot_microbench_'"${STAMP}"'.json \
      2>&1 | tee /logs/moonshot_microbench_'"${STAMP}"'.log
  '
