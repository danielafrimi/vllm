...
class InternHVLImagePixelInputs(TypedDict):
...
class InternHVLImageEmbeddingInputs(TypedDict):
...
class InternHVLVideoEmbeddingInputs(TypedDict):
...
class BaseInternHVLProcessor(ABC):
    """
    This model doesn't define its own HF processor,
    so we implement our own one here.

    The code to insert image tokens is based on:
    https://huggingface.co/OpenGVLab/InternHVL2-1B/blob/main/modeling_internvl_chat.py#L252
    """
...
class InternHVLProcessor(BaseInternHVLProcessor):
    """
    HF Processor for InternHVLChatModel with extended video processing logic.

    Code for video processing is adapted from video example:
    https://huggingface.co/OpenGVLab/InternHVL3-1B#inference-with-transformers
    """
...
class BaseInternHVLProcessingInfo(BaseProcessingInfo):
    """Basic image-only ProcessingInfo for InternHVL-style models."""
    ...
    def get_num_image_tokens(
        ...
        *,
        image_width: int,
        image_height: int,
        processor: Optional[BaseInternHVLProcessor],
    ) -> int:
...
_I = TypeVar("_I", bound=BaseInternHVLProcessingInfo)


class BaseInternHVLDummyInputsBuilder(BaseDummyInputsBuilder[_I]):
    """Basic image-only DummyInputsBuilder for InternHVL-style models."""
...
class InternHVLProcessingInfo(BaseInternHVLProcessingInfo):
    """InternHVL ProcessingInfo extended for video processing"""
...
class InternHVLDummyInputsBuilder(
        BaseInternHVLDummyInputsBuilder[InternHVLProcessingInfo]):
    """InternHVL DummyInputsBuilder extended for video support"""
...
class InternHVLMultiModalProcessor(
        BaseInternHVLMultiModalProcessor[InternHVLProcessingInfo]):
    """InternHVL MultiModalProcessor extended for video support"""
...
class NemotronH_Nano_VL(nn.Module, HasInnerState, IsHybrid, SupportsMultiModal,
    ...
    def _parse_and_validate_image_input(
            self, **kwargs: object) -> Optional[InternHVLImageInputs]:
        ...
        if image_embeds is not None:
            ...
            return InternHVLImageEmbeddingInputs(
                type="image_embeds",
        ...
        if pixel_values_flat is not None:
            ...
            return InternHVLImagePixelInputs(
                type="pixel_values",
    ...
    def _parse_and_validate_video_input(
            self, **kwargs: object) -> Optional[InternHVLVideoPixelInputs]:
        ...
        if pixel_values_flat_video is not None:
            ...
            return InternHVLVideoPixelInputs(
    ...
    def _process_image_input(
        self,
        image_input: Union[InternHVLImageInputs, InternHVLVideoPixelInputs],
    ) -> tuple[torch.Tensor, ...]:
