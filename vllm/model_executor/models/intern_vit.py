...
from vllm.model_executor.models.input_conditionerV1 import (
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
            ...
            bias: bool = True,
            strict_img_size:
        bool = True,  # todo where do we pass this in config ??
            no_embed_class:
        bool = False,  # todo where do we pass this in config ??
            dynamic_img_pad:
        bool = False,  # todo where do we pass this in config ??
        ...
        self.reg_tokens = nn.Parameter(
            torch.zeros(1, config.reg_tokens,
                        config.hidden_size)) if config.reg_tokens else None
        ...
        if not pos_embed or pos_embed == 'none':
            ...
            self.pos_embed = nn.Parameter(
                torch.randn(1, embed_len, config.hidden_size) * .02)
        ...
        self.proj = nn.Conv2d(3,
                              ...
                              bias=bias)
        self.norm = NORM2FN[config.norm_type](
            config.hidden_size) if config.norm_type else nn.Identity()
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.dynamic_img_size:
            ...
            pos_embed = resample_abs_pos_embed(
                ...
                num_prefix_tokens=0
                if self.no_embed_class else self.num_prefix_tokens,
            )
            x = x.view(B, -1, C)
        else:
    ...
    def __init__(
        ...
        *,
        ...
        self.patch_generator = ViTPatchGenerator(
            ...
            cls_token=True,
            register_multiple=config.args["register_multiple"])
...
class RadioModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.model = InternVisionModelModified(
            config=config,
            quant_config=quant_config,
            num_hidden_layers_override=num_hidden_layers_override,
            num_dummy_heads=num_dummy_heads,
