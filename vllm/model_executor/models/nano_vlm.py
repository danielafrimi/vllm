# --------------------------------------------------------
# Adapted from https://github.com/vllm-project/vllm/blob/main/vllm/model_executor/models/internvl.py under Apache-2.0 License
...
from transformers import AutoModel, BatchEncoding, PretrainedConfig, TensorType, AutoConfig

from vllm import ModelRegistry, LLM, SamplingParams
from vllm.config import VllmConfig
from vllm.multimodal import MULTIMODAL_REGISTRY
from vllm.multimodal.inputs import NestedTensors, MultiModalFieldConfig, MultiModalKwargs, MultiModalDataDict
from vllm.multimodal.parse import ImageEmbeddingItems, ImageProcessorItems, ImageSize, MultiModalDataItems
from vllm.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, PromptUpdate, PromptUpdateDetails, PromptReplacement
...
from vllm.model_executor.models.interfaces import MultiModalEmbeddings, SupportsMultiModal, SupportsV0Only
...
def input_conditioner(x, norm_mean, norm_std):
    # print("INPUT COND", norm_mean.dtype, norm_std.dtype)
...
def image_to_pixel_values(
    ...
    *,
    ...
    # pixel_values = torch.stack([image for image in images]) * rescale_factor
...
class BaseCustomProcessor(ABC):
    ...
    def _preprocess_image(
        ...
        if len(images) == 0:
            ...
            # print("PIXEL values:", torch.cat(pixel_values_lst).shape, torch.tensor([len(item) for item in pixel_values_lst]))
            # print("===============", torch.cat(pixel_values_lst).shape, torch.cat(pixel_values_lst).dtype, self.norm_mean.shape,  self.norm_std.shape)
            image_inputs: dict[str, NestedTensors] = {
                "pixel_values_flat":
                input_conditioner(torch.cat(pixel_values_lst), self.norm_mean, self.norm_std),
...
class CustomProcessor(BaseCustomProcessor):
    ...
    def __call__(
        ...
        text, images = [
            self._make_batch_input(x) for x in (text, images)
        ]
...
class NemotronH_Nano_VL(nn.Module, HasInnerState, IsHybrid, SupportsMultiModal, SupportsV0Only):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.vision_model.radio_model.make_preprocessor_external() # todo check this as well 
    ...
    def _process_image_input(self, image_input: CustomImageInputs) -> torch.Tensor:
        # TODO(amalasanjayd)
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        #         *self.language_model._get_mamba_cache_shape())
    ...
    def print_architecture(self, detailed: bool = True, save_to_file: str = None):
        ...
        try:
            print("=" * 100)
            print(f"NemotronH_Nano_VL Model Architecture")
            print("=" * 100)
            ...
            for name, param in self.named_parameters():
                ...
                if detailed:
                    print(f"{name:<70} | Shape: {str(param.shape):<25} | Size: {param_size:>12,} | Dtype: {param.dtype}")
            ...
            for component, params in param_groups.items():
                if params:  # Only show components that have parameters
                    ...
                    print(f"{component:<20} | Parameters: {len(params):>4} | Total Size: {component_total:>15,} | {percentage:>6.2f}%")
...
def register():
    ModelRegistry.register_model("NemotronH_Nano_VL", NemotronH_Nano_VL)
