...
class BaseCustomProcessor(ABC):
    ...
    def _preprocess_image(
        ...
        if len(images) == 0:
            ...
            for t in text:
                parts = t.split('<image>')
                assert len(parts) - 1 == len(
                    pixel_values_lst
                ), f"Number of <image> tokens ({len(parts) - 1}) doesn't match num_patches_list length ({len(pixel_values_lst)})"
...
@MULTIMODAL_REGISTRY.register_processor(CustomMultiModalProcessor,
...
class NemotronH_Nano_VL(nn.Module, HasInnerState, IsHybrid, SupportsMultiModal,
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Move input normalization to processor to mirror original HF implementation where norm is done on fp32
        ...
        try:
            ...
            for component, params in param_groups.items():
                if params:  # Only show components that have parameters
                    ...
                    print(
                        f"{component:<20} | Parameters: {len(params):>4} | Total Size: {component_total:>15,} | {percentage:>6.2f}%"
                    )
